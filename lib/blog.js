...
export async function getBlogPost(slug) {
  ...
  if (!responseCurrentPost.ok) {
    throw new Error(`CMS returned ${responseCurrentPost.status} for ${urlCurrentPost}`);
  }
  ...
  const currentPostPublishedAt = attributes.publishedAt;
  ...
  const previousPost = bodyPreviousPost.data[0]
    ? bodyPreviousPost.data[0].attributes
    : null;
  ...
  const nextPost = bodyNextPost.data[0] ? bodyNextPost.data[0].attributes : null;
